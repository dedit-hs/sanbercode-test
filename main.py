...
@app.post("/daftar")
async def tambah_nasabah(nasabah: SchemaNasabah):
...
@app.post("/tabung")
async def nasabah_menabung(transaksi: SchemaTransaksi):
...
@app.post("/tarik")
async def nasabah_menarik_tabungan(transaksi: SchemaTransaksi):
    cek_rekening = get_rekening(transaksi.rekening)
    if not cek_rekening:
        return {"remark": "No rekening salah"}
...
@app.get("/saldo/{no_rek}")
async def get_saldo(no_rek: str):
